...
risk_IT_A_B <- 0.01
risk_IT_A_C <- 0.001
risk_IT_A_C_excl <- risk_IT_A_B + risk_IT_A_C
risk_IT_A_D <- 0.0001
risk_IT_A_D_excl <- risk_IT_A_C_excl + risk_IT_A_D
risk_IT_B_C <- 0.05
risk_IT_B_D <- 0.02
risk_IT_B_D_excl <- risk_IT_B_C + risk_IT_B_D
risk_IT_C_D <- 0.06
risk_IT_D_death <- 0.1
...
create_population <- function(status){ # samo iscemo steady state populacijo brez screeninga in zdravljenja
    ...
    if (random_death < risk_death){
      ...
      status_copy$treatment_status[i] <- "dead"
        ...
        if (status$risk[i]=="low"){
          ...
          } else if (status$stage[i]=="2"){
              if (random < risk_low_2_3){
              status_copy$stage[i] <- "3"
            } else if (random < (risk_low_2_A_excl)){
              status_copy$stage[i] <- "A"
            } 
          } else if (status$stage[i]=="3"){
              if (random < risk_low_3_A){
              status_copy$stage[i] <- "A"
            }
          } else if (status$stage[i]=="A"){
              if (random < risk_low_A_B){
              status_copy$stage[i] <- "B"
            }
          } else if (status$stage[i]=="B"){
              if (random < risk_low_B_C){
              status_copy$stage[i] <- "C"
            }
          } else if (status$stage[i]=="C"){
              if (random < risk_low_C_D){
              status_copy$stage[i] <- "D"
            }
          } else if (status$stage[i]=="D"){
  ...
  new <- trunc(rnorm(1, risk_birth, 0.01) * n)
...
count_function <- function(status){
  ...
  test <- setkey(table, stage, risk, surv_year, treatment_year, treatment_status)[, .N, 
               by="stage,risk,surv_year,treatment_year,treatment_status"][CJ(c("nought", "1", "2", "3", "A", "B", "C", "D", "dead_AC", "dead_other"),
               c("low", "high"),
    ...
    if (random_death < risk_death){
      ...
      status_copy$treatment_status[i] <- "dead"
        ...
        if (status$treatment_status[i]=="IT" | status$treatment_status[i]=="repeat_IT"){ #vse AIN imajo treatment_status AIN_history, in se tu obravnavajo kot "low" in "high" risk
          if (status$stage[i]=="A"){
            if (random < risk_IT_A_B){
              staus_copy$stage[i] <- "B"
            } else if (random < risk_IT_A_C_excl){
              status_copy$stage[i] <- "C"
            } else if (random < risk_IT_A_D_excl){
              status_copy$stage[i] <- "D"
            } else if (random <- risk_IT_A_D_excl + 0.8){# ti so ozdraveli in zacnejo steti leta za followup
              status_copy$stage[i] <- "nought"
              status_copy$treatment_status[i] <- "CA_followup"
              status_copy$treatment_year[i] <- 1
            }
          } else if (status$stage[i]=="B"){
            if (random < risk_IT_B_C){
              status_copy$stage[i] <- "C"
            } else if (random < risk_IT_B_D_excl){
              status_copy$stage[i] <- "D"
            } else if (random <- risk_IT_A_D_excl + 0.5){# ti so ozdraveli in zacnejo steti leta za followup
              status_copy$stage[i] <- "nought"
              status_copy$treatment_status[i] <- "CA_followup"
              status_copy$treatment_year[i] <- 1
            }
          } else if (status$stage[i]=="C"){
            if (random < risk_IT_C_D){
              status_copy$stage[i] <- "D"
            } else if (random <- risk_IT_A_D_excl + 0.1){# ti so ozdraveli in zacnejo steti leta za followup
              status_copy$stage[i] <- "nought"
              status_copy$treatment_status[i] <- "CA_followup"
              status_copy$treatment_year[i] <- 1
            }
          } else if (status$stage[i]=="D"){
            if (random < risk_IT_D_death){
  ...
  for (i in 1:dim(status)[1]){
    if (status$treatment_status[i]!="dead"){
      ...
      if (status$treatment_status[i]=="CA_followup"){#gledamo vsako leto, najdemo vse; "IT" tudi gledamo vsako leto, ampak najden AIN ne spremeni nicesar, ker ga samo odstranimo in itak ni generiran in gre zdravljenje naprej, ca pa ne bomo nasli, ker ga gledamo vsako leto
        if (status$stage[i] =="A" | status$stage[i]=="B" | status$stage[i]=="C"| status$stage[i]=="D"){
        ...
        if (status$surv_year[i]==0){ #te screenamo
          if (status$stage[i]=="1"){
            if (random < find_1_screen){
              if(status$treatment_status[i]=="none")
              status$treatment_status[i] <- "AIN"
            }
          } else if (status$stage[i]=="2"){
            if (random < find_2_screen){
              if(status$treatment_status[i]=="none")
              status$treatment_status[i] <- "AIN"
            }
          } else if(status$stage[i]=="3"){
            if (random < find_3_screen){
              if(status$treatment_status[i]=="none")
              status$treatment_status[i] <- "AIN"
            }
          } else if (status$stage[i]=="A"){
            if (random < find_A_screen){
              status$treatment_status[i] <- "IT"
              status$treatment_year[i] <- 0
              status$surv_year[i] <- NA
            }
          } else if (status$stage[i]=="B"){
            if (random < find_B_screen){
              status$treatment_status[i] <- "IT"
              status$treatment_year[i] <- 0
              status$surv_year[i] <- NA
            }
          } else if (status$stage[i]=="C"){
            if (random < find_C_screen){
              status$treatment_status[i] <- "IT"
              status$treatment_year[i] <- 0
              status$surv_year[i] <- NA
            }
          } else if (status$stage[i]=="D"){
            if (random < find_D_screen){
              status$treatment_status[i] <- "IT"
              status$treatment_year[i] <- 0
              status$surv_year[i] <- NA
            }
            ...
            if (status$stage[i]=="A"){
              if (random < find_A_null){
                status$treatment_status[i] <- "IT"
                status$treatment_year[i] <- 0
                status$surv_year[i] <- NA
              }
            } else if (status$stage[i]=="B"){
              if (random < find_B_null){
                status$treatment_status[i] <- "IT"
                status$treatment_year[i] <- 0
                status$surv_year[i] <- NA
              }
            } else if (status$stage[i]=="C"){
              if (random < find_C_null){
                status$treatment_status[i] <- "IT"
                status$treatment_year[i] <- 0
                status$surv_year[i] <- NA
              }
            } else if (status$stage[i]=="D"){
              if (random < find_D_null){
                status$treatment_status[i] <- "IT"
                status$treatment_year[i] <- 0
                status$surv_year[i] <- NA
              }
...
find_1_null <- 0
find_2_null <- 0
find_3_null <- 0 # ce to dvignes, moras spremeniti kodo
find_A_null <- 0.1
find_B_null <- 0.4
find_C_null <- 0.8
find_D_null <- 1
find_1_screen <- 0.8 # od tu naprej rabis tri sete, za boljsi in slabsi screen in sploh brez screena
find_2_screen <- 0.95
find_3_screen <- 1
find_A_screen <- 1
find_B_screen <- 1
