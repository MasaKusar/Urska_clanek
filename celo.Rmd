...
risk_low_nought_low <- 0.0001
risk_low_nought_high <- 0.0001
risk_low_nought_high_excl <- risk_low_nought_low + risk_low_nought_high
risk_low_nought_ca <- 0.000001
risk_low_nought_ca_excl <- risk_low_nought_high_excl + risk_low_nought_ca
risk_low_low_high <- 0.0001
risk_low_low_ca <- 0.00002
risk_low_low_ca_excl <- risk_low_low_high + risk_low_low_ca
risk_low_high_ca <- 0.0001

risk_high_nought_low <- 0.0002
risk_high_nought_high <- 0.0002
risk_high_nought_high_excl <- risk_high_nought_low + risk_high_nought_high
risk_high_nought_ca <- 0.000002
risk_high_nought_ca_excl <- risk_high_nought_high_excl + risk_high_nought_ca
risk_high_low_high <- 0.0002
risk_high_low_ca <- 0.00004
risk_high_low_ca_excl <- risk_high_low_high + risk_high_low_ca
risk_high_high_ca <- 0.002

risk_ca_death <- 0.2

risk_IT_ca_death <- 0.1
      ...
      status_copy$treatment_status[i] <- "dead"
        ...
        if (status$risk[i]=="low"){
          if (status$stage[i]=="nought"){
            if (random < risk_low_nought_low){
              status_copy$stage[i] <- "low"
            } else if (random < (risk_low_nought_high_excl)){
              status_copy$stage[i] <- "high"
            } else if (random < (risk_low_nought_ca_excl)){
              status_copy$stage[i] <- "ca"
            }
          } else if (status$stage[i]=="low"){
              if (random < risk_low_low_high){
              status_copy$stage[i] <- "high"
            } else if (random < (risk_low_low_ca_excl)){
              status_copy$stage[i] <- "ca"
            }
          } else if (status$stage[i]=="high"){
              if (random < (risk_low_high_ca_excl)){
              status_copy$stage[i] <- "ca"
            } 
          } else if (status$stage[i]=="ca"){
              if (random < risk_ca_death){
              status_copy[i] <- "dead_AC"
            }
          }
      } else if (status$risk[i]=="high"){ #zaklepaj low risk
          if (status$stage[i]=="nought"){
            if (random < risk_high_nought_low){
              status_copy$stage[i] <- "low"
            } else if (random < (risk_high_nought_high_excl)){
              status_copy$stage[i] <- "high"
            } else if (random < (risk_high_nought_ca_excl)){
              status_copy$stage[i] <- "ca"
            }
          } else if (status$stage[i]=="low"){
              if (random < risk_high_low_high){
              status_copy$stage[i] <- "high"
            } else if (random < (risk_high_low_ca_excl)){
              status_copy$stage[i] <- "ca"
            }
          } else if (status$stage[i]=="high"){
              if (random < (risk_high_high_ca_excl)){
              status_copy$stage[i] <- "ca"
            } 
          } else if (status$stage[i]=="ca"){
              if (random < risk_ca_death){
          ...
          if (status$stage[i]=="ca"){
            ...
            if (status$risk[i]=="low"){
              if (status$stage[i]=="nought"){
                if (random < risk_low_nought_low){
                  status_copy$stage[i] <- "low"
                } else if (random < (risk_low_nought_low_excl)){
                  status_copy$stage[i] <- "high"
                } else if (random < (risk_low_nought_ca_excl)){
                  status_copy$stage[i] <- "ca"
                }
...
find_low_screen <- 0.8 # od tu naprej rabis tri sete, za boljsi in slabsi screen in sploh brez screena
find_high_screen <- 0.95
find_ca_screen <- 1
```
